...
function quests.hide_hud(playername)
	...
	for _,quest in pairs(quests.hud[playername].list) do
		...
		player:hud_remove(quest.id_item)
		player:hud_remove(quest.id_action)
...
function quests.update_hud(playername)
	...
	for j,hud_element in ipairs(quests.hud[playername].list) do
		...
		if (i ~= j) then
			player:hud_change(hud_element.id, "offset", 
					{ x= hud_config.offset.x, 
					y=hud_config.offset.y + (i-1) *40})
			player:hud_change(hud_element.id_item, "offset", 
					{ x= hud_config.offset.x-35, 
					y=hud_config.offset.y + (i-1) *40})
			player:hud_change(hud_element.id_action, "offset", 
					{ x= hud_config.offset.x-70, 
					y=hud_config.offset.y + (i-1) *40})
		...
		i = i + 1
		    ...
		    player:hud_remove(hud_element.id_item)
		    player:hud_remove(hud_element.id_action)
		    if (hud_element.id_background ~= nil) then
	...
	for questname,questspecs in pairs(quests.active_quests[playername]) do
		if (not visible[questname]) then
			...
			local id_item_text = "quests_default_hud_image.png"
			if quest.image ~= nil then
				id_item_text = quest.image
			end
			local id_item = player:hud_add(
					{ hud_elem_type = "image",
					 scale = { x = 1, y = 1 },
					 size = { x = 2, y = 4 },
					 alignment = { x = 1, y = 1 },
					 position = { x = hud_config.position.x, y = hud_config.position.y },
					 offset = { x = hud_config.offset.x-35, 
					 		y = hud_config.offset.y + counter *40},
					 text = id_item_text,
					 --size = {x=32, y=32},
					 scale = {x=2, y=2},
				 	})
			local id_action_text = "quests_unknown_action.png"
			if quest.type ~= nil then
				id_action_text = "quests_"..quest.type..".png"
			end

			local id_action = player:hud_add(
					{ hud_elem_type = "image",
					 scale = { x = 1, y = 1 },
					 size = { x = 2, y = 4 },
					 alignment = { x = 1, y = 1 },
					 position = { x = hud_config.position.x, y = hud_config.position.y },
					 offset = { x = hud_config.offset.x-70, 
					 	y = hud_config.offset.y + counter * 40},
					 text = id_action_text, 
					 --size = {x=32, y=32},
					 scale = {x=2, y=2},
			...
			if (quests.registered_quests[questname].max ~= 1) then
				...
				id_bar = player:hud_add(
					{hud_elem_type = "image",
					 scale = { x = math.floor(20 * questspecs.value /
					 	   quests.registered_quests[questname].max),y = 1 },
					 alignment = { x = 1, y = 1 },
					 position = { x = hud_config.position.x, y = hud_config.position.y },
					 offset = { x = hud_config.offset.x + 2, 
					 	    y = hud_config.offset.y + counter * 40 + 24 },
