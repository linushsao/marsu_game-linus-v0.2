...
minetest.register_node("marsair:airmaker", {
	...
	on_construct = function(pos)
		...
		local inv = meta:get_inventory()
		inv:set_size("main", 4)
		minetest.get_node_timer(pos).start(marsairconfig.flower_air_time)
	end,
	on_timer = function(pos, elapsed)
		if random(marsairconfig.flower_air_chance) == 1 then
			marsair.clean(pos)
			minetest.get_node_timer(pos).start(marsairconfig.flower_air_time)
		end
	end,
...
minetest.register_node("marsair:tree_air_cleaner", {
	...
	on_construct = function(pos)
		...
		local inv = meta:get_inventory()
		inv:set_size("main", 4)
		minetest.get_node_timer(pos).start(marsairconfig.tree_air_maker_time)
        end,
        on_timer = function(pos, elapsed)
